    ...
    if (numData[numData.length - 1].isEmpty && numData[numData.length - 2].num == numData.length - 1) {
      ...
      if (flg) {
        ...
        wx.showModal({
          ...
          this.setData({//存下最快纪录
            fm : (parseInt(this.data.fm)*60+parseInt(this.data.fs))>=(parseInt(m)*60+parseInt(s)) ? m:fm,
            fs : (parseInt(this.data.fm)*60+parseInt(this.data.fs))>=(parseInt(m)*60+parseInt(s)) ? s:fs
            })
      }
    }
  },
